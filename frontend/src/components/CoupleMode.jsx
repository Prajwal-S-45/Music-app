...
function CoupleMode({ user, onPlayTrack, onQueueTrack }) {
  ...
  const isHost = Boolean(socketId && (roomDetails?.hostSocketId ? socketId === roomDetails.hostSocketId : partner1?.socketId === socketId));
    ...
    const unsubs = [
      ...
      subscribe('seek', (data) => {
        if (typeof data.currentTime === 'number') {
          ...
          const localTime = audio ? audio.currentTime : currentTime;
      ...
      subscribe('room_sync', (data) => {
        ...
        const audio = audioRef.current;
        const localTime = audio ? audio.currentTime : currentTime;
        const drift = Math.abs(localTime - (data.currentTime || 0));
        if (drift > 0.4) {
          seekAudioTo(data.currentTime || 0);
        }
        if (typeof data.isPlaying === 'boolean') setIsPlaying(data.isPlaying);
  ...
  useEffect(() => {
    ...
    const onLoadedMetadata = () => {
      ...
      if (pendingSeekRef.current !== null) {
        ...
        try { audio.currentTime = pendingSeekRef.current; } catch {}
  ...
  // Heartbeat sync - only host (authoritative sender) emits periodic heartbeat
  useEffect(() => {
    if (!socket || !roomId || !isPlaying || !isHost) return;
  ...
  }, [socket, roomId, isPlaying, isHost, emitEvent]);
    ...
    <>
      ...
      {screen === 'landing' ? (
        ...
        <div className="couple-active" style={{ backgroundImage: songCover ? `url(${songCover})` : `url(${COUPLE_BG})` }}>
          ...
          {errorMsg && (
            <div className="couple-landing__error" style={{ position: 'relative', zIndex: 2, maxWidth: '500px', margin: '10px 20px 0' }}>
              {errorMsg}
            </div>
          )}
